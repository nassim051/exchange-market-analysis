...
class GateioWaveAnalyzer(AbstracyWaveAnalyzer.AbstractWaveAnalyzer):
        def __init__(self,numProcess,numOfThreads,nbHour,timeFrame,waveVolatility,period,key=1):
                self.key=key
                super().__init__(period=period,basicDataMan=BasicDataMan.GateBasicDataMan(),marketMan=MarketMan.GateMarketMan(),exchange='gateio',secondOrMili=1,numProcess=numProcess,numOfThreads=numOfThreads,nbHour=nbHour,timeFrame=timeFrame,waveVolatility=waveVolatility)
